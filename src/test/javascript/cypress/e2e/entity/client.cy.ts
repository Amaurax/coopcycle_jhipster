...
describe('Client e2e test', () => {
  ...
  const clientSample = {
    lastName: 'Fleury',
    firstName: 'Aquiline',
    email: 'jjhSR@4cz.LRwGo',
    phone: '8266897615',
    address: 'calculating 24/7',
  };
  ...
  describe('new Client page', () => {
    ...
    it('should create an instance of Client', () => {
      cy.get(`[data-cy="lastName"]`).type('Fleury').should('have.value', 'Fleury');

      cy.get(`[data-cy="firstName"]`).type('Manassé').should('have.value', 'Manassé');

      cy.get(`[data-cy="email"]`).type('0Z6k8@cH176.sm').should('have.value', '0Z6k8@cH176.sm');

      cy.get(`[data-cy="phone"]`).type('7070577105').should('have.value', '7070577105');

      cy.get(`[data-cy="address"]`).type('deploy interactive b').should('have.value', 'deploy interactive b');
