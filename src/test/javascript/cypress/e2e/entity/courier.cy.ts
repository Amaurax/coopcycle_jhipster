...
describe('Courier e2e test', () => {
  ...
  const courierSample = { lastName: 'Moreau', firstName: 'Maureen', email: 'Iy6rA@y3.YfiIzPt', phone: '2488774953' };
  ...
  describe('new Courier page', () => {
    ...
    it('should create an instance of Courier', () => {
      cy.get(`[data-cy="lastName"]`).type('Sanchez').should('have.value', 'Sanchez');

      cy.get(`[data-cy="firstName"]`).type('Pépin').should('have.value', 'Pépin');

      cy.get(`[data-cy="email"]`).type('BTY@1.OiHqvFr').should('have.value', 'BTY@1.OiHqvFr');

      cy.get(`[data-cy="phone"]`).type('9896555187').should('have.value', '9896555187');

      cy.get(`[data-cy="vehicle"]`).select('BIKE');

      cy.get(`[data-cy="status"]`).type('Surinam Intuitive').should('have.value', 'Surinam Intuitive');
