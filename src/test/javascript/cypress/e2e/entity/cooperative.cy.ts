...
describe('Cooperative e2e test', () => {
  ...
  const cooperativeSample = { name: 'Congolese Rhône-Alpes cross-platform' };
  ...
  describe('new Cooperative page', () => {
    ...
    it('should create an instance of Cooperative', () => {
      ...
      cy.get(`[data-cy="address"]`).type('interface').should('have.value', 'interface');

      cy.get(`[data-cy="director"]`).type('ADP quantifying').should('have.value', 'ADP quantifying');
